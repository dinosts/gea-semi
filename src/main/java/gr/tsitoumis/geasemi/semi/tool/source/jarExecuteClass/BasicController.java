...
import gr.tsitoumis.geasemi.semi.tool.source.gui.Analyser;
import gr.tsitoumis.geasemi.semi.tool.source.gui.MethodOppExtractor;
import gr.tsitoumis.geasemi.semi.tool.source.gui.MethodOppExtractorSettings;
import gr.tsitoumis.geasemi.semi.tool.source.parsers.CodeFile;
import gr.tsitoumis.geasemi.semi.tool.source.parsers.cFile;
import gr.tsitoumis.geasemi.semi.tool.source.splitlongmethod.JavaClass;
import gr.tsitoumis.geasemi.semi.tool.source.splitlongmethod.Method;
            ...
            for (File file : fList) {
                if (file.isFile() && file.getName().contains(".") && file.getName().charAt(0) != '.') {
                    ...
                    if (str[str.length - 1].equalsIgnoreCase("java")) {
                        ...
                        // }
                        parser.parse();
                        Utilities.writeCSV("./" + file.getName() + "_original_parsed.txt", parser.getOutput(),
                                false);
                        Utilities.writeCSV("./" + file.getName() + "_parsed.txt", parser.getOutput(), false);
